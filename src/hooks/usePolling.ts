/** ポーリングのステートマシン。idle→running→paused/slowmode の状態遷移とタイマー管理 */
...
export function usePolling(
  ...
  const executePollRef = useRef<() => Promise<void>>(null!);
  ...
  const schedule = useCallback((sec: number) => {
    ...
    intervalIdRef.current = setInterval(() => { executePollRef.current?.(); }, sec * 1000);
  }, []);
  ...
  useEffect(() => { executePollRef.current = executePoll; }, [executePoll]);

/** アプリ全体のオーケストレーター。開始・停止・ポーリングの制御フローを統括する */
...
export function useAppController() {
  ...
  const handlePollingTick = useCallback(async () => {
    try {
      ...
      const msg = error instanceof Error ? error.message : 'ポーリング中に問題が発生しました';
      setStatus(`エラー: ${msg}`);
      showToast(msg, 'error');
      pauseRef.current();
      return null;
  ...
  const handleStart = useCallback(async () => {
    ...
    try {
      ...
      const msg = error instanceof Error ? error.message : '開始中にエラーが発生しました';
      setStatus(`エラー: ${msg}`);
      showToast(msg, 'error');
    }

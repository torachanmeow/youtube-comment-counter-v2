/** チャットメッセージのバッチ処理。メッセージ分類・キーワード検出・統計値の蓄積を行う */
  ...
  const processMessages = useCallback(
    (messages: YouTubeAPIChatMessage[], initialStats: Partial<Stats> = {}): number => {
      ...
      for (const msg of messages) {
        ...
        accumulated.memberJoins += processed.contributions.memberJoins;
        accumulated.memberMilestones += processed.contributions.memberMilestones;

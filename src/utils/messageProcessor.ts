/** チャットメッセージの解析。スパチャ円換算・キーワード検出・重複制御・メッセージ分類を行う純粋関数群 */
...
export function processMessage(
  ...
  const text = stripHTML(msg.snippet.displayMessage || '');
  ...
  const result: ProcessedMessage = {
    ...
    type: 'normal',
    highlights: [],
    contributions: { superChat: 0, superSticker: 0, memberJoins: 0, memberMilestones: 0, keywords: {}, rawKeywords: {} },
  };
  ...
  if (msgType === 'newSponsorEvent') {
    ...
    result.contributions.memberJoins = 1;
    ...
    result.type = 'membership';
    result.contributions.memberMilestones = 1;
    result.highlights.push('membership');
    ...
    result.contributions.memberJoins = giftCount;
    result.highlights.push('membership');
    ...
    // 受け取り側は送信側の membershipGiftingEvent で集計済みのためカウントしない
    result.type = 'membership';
    result.highlights.push('membership');
